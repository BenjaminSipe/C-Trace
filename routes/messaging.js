...
function releaseEmailTemplate(data) {
  return {
    to: data.to, // list of receivers
    subject: "C-Trace Quarantine Release", // Subject line
    text:
      "Hello " +
      data.name +
      ", this email is sent to notify you that your quarantine period is officially over." +
      "\nIf you believe this email was sent in error, " +
      "please contact your local health authority for verification." +
      "\n-your local health authority,\n" +
      "This email was generated by C-Trace. For more information go to C-trace.com or contact c.trace.contact@gmail.com.",
  ...
  if (req.params.id) {
    ...
    getCollection(async (collection) => {
      ...
      switch (person?.status) {
        case "Exposed":
          reassignment = { $set: { status: "Past" } };
          break;
        case "Positive":
          reassignment = { $set: { status: "Recovered" } };
          break;
        case "Recovered":
        case "Past":
          b = false;
          res.status(201).send({
            err: "Patient has already been released from quarantine.",
          });
          break;
        default:
          b = false;
      ...
      var touch = await collection.updateOne(query, reassignment);
      if (person && touch && b) {
        if (person.phone) {
          ...
          let query = {
            body:
              "Hello " +
              person.name +
              ", this message is sent to notify you that your quarantine period is officially over." +
              "\nIf you have questions, please contact c.trace.contact@gmail.com for more information.",
            to:
              "+1" +
              person.phone
                .split("")
          ...
          client.messages.create(query).then((message) =>
            res.send({
              sid: message.sid,
          ...
          if (person.email) {
            let emailData = releaseEmailTemplate({
